// htmx.org@2.0.8 downloaded from https://ga.jspm.io/npm:htmx.org@2.0.8/dist/htmx.esm.js
...
/**
   * @param {string} resp
   * @returns {Document}
   */function parseHTML(e){if("parseHTMLUnsafe"in Document)return Document.parseHTMLUnsafe(e);const t=new DOMParser;return t.parseFromString(e,"text/html")}
/**
...
/**
   ...
   * @param {boolean=} global
   * @returns {(Node|Window)[]}
   */function querySelectorAllExt(e,t,n){if(t.indexOf("global ")===0)return querySelectorAllExt(e,t.slice(7),true);e=resolveTarget(e);const r=[];{let e=0;let n=0;for(let o=0;o<t.length;o++){const s=t[o];if(s!==","||e!==0)s==="<"?e++:s==="/"&&o<t.length-1&&t[o+1]===">"&&e--;else{r.push(t.substring(n,o));n=o+1}}n<t.length&&r.push(t.substring(n))}const o=[];const s=[];while(r.length>0){const t=normalizeSelector(r.shift());let i;t.indexOf("closest ")===0?i=closest(asElement(e),normalizeSelector(t.slice(8))):t.indexOf("find ")===0?i=find(asParentNode(e),normalizeSelector(t.slice(5))):t==="next"||t==="nextElementSibling"?i=asElement(e).nextElementSibling:t.indexOf("next ")===0?i=scanForwardQuery(e,normalizeSelector(t.slice(5)),!!n):t==="previous"||t==="previousElementSibling"?i=asElement(e).previousElementSibling:t.indexOf("previous ")===0?i=scanBackwardsQuery(e,normalizeSelector(t.slice(9)),!!n):t==="document"?i=document:t==="window"?i=window:t==="body"?i=document.body:t==="root"?i=getRootNode(e,!!n):t==="host"?i=/** @type ShadowRoot */e.getRootNode().host:s.push(t);i&&o.push(i)}if(s.length>0){const t=s.join(",");const r=asParentNode(getRootNode(e,!!n));o.push(...toArray(r.querySelectorAll(t)))}return o}
/**
   * @param {Node} start
   * @param {string} match
   * @param {boolean} global
...
/**
   ...
   */function cloneAttributes(e,t){forEach(Array.from(e.attributes),(function(n){!t.hasAttribute(n.name)&&shouldSettleAttribute(n.name)&&e.removeAttribute(n.name)}));forEach(t.attributes,(function(t){shouldSettleAttribute(t.name)&&e.setAttribute(t.name,t.value)}))}
/**
...
/** @type HtmxSwapStyle */let s="outerHTML";if(e==="true");else if(e.indexOf(":")>0){s=e.substring(0,e.indexOf(":"));o=e.substring(e.indexOf(":")+1)}else s=e;t.removeAttribute("hx-swap-oob");t.removeAttribute("data-hx-swap-oob");const i=querySelectorAllExt(r,o,false);if(i.length){forEach(i,(function(e){let r;const o=t.cloneNode(true);r=getDocument().createDocumentFragment();r.appendChild(o);isInlineSwap(s,e)||(r=asParentNode(o));const i={shouldSwap:true,target:e,fragment:r};if(triggerEvent(e,"htmx:oobBeforeSwap",i)){e=i.target;if(i.shouldSwap){handlePreservedElements(r);swapWithStyle(s,e,e,r,n);restorePreservedElements()}forEach(n.elts,(function(e){triggerEvent(e,"htmx:oobAfterSwap",i)}))}}));t.parentNode.removeChild(t)}else{t.parentNode.removeChild(t);triggerErrorEvent(getDocument().body,"htmx:oobErrorNoTarget",{content:t})}return e}function restorePreservedElements(){const e=find("#--htmx-preserve-pantry--");if(e){for(const t of[...e.children]){const e=find("#"+t.id);e.parentNode.moveBefore(t,e);e.remove()}e.remove()}}
/**
...
/**
   ...
   * @param {HtmxSettleInfo} settleInfo
   */function handleAttributes(e,t,n){forEach(t.querySelectorAll("[id]"),(function(t){const r=getRawAttribute(t,"id");if(r&&r.length>0){const o=r.replace("'","\\'");const s=t.tagName.replace(":","\\:");const i=asParentNode(e);const a=i&&i.querySelector(s+"[id='"+o+"']");if(a&&a!==i){const e=t.cloneNode();cloneAttributes(t,a);n.tasks.push((function(){cloneAttributes(t,e)}))}}}))}
/**
   * @param {Node} child
...
/**
   ...
   */function maybeGenerateConditional(e,t,n){if(t[0]==="["){t.shift();let r=1;let o=" return (function("+n+"){ return (";let s=null;while(t.length>0){const i=t[0];if(i==="]"){r--;if(r===0){s===null&&(o+="true");t.shift();o+=")})";try{const t=maybeEval(e,(function(){return Function(o)()}),(function(){return true}));t.source=o;return t}catch(e){triggerErrorEvent(getDocument().body,"htmx:syntax:error",{error:e,source:o});return null}}}else i==="["&&r++;isPossibleRelativeReference(i,s,n)?o+="(("+n+"."+i+") ? ("+n+"."+i+") : (window."+i+"))":o+=i;s=t.shift()}}}
/**
   * @param {string[]} tokens
   * @param {RegExp} match
   * @returns {string}
...
/**
   ...
   * @param {Element} elt
   * @returns {boolean}
   */function shouldCancel(e,t){if(e.type==="submit"&&t.tagName==="FORM")return true;if(e.type==="click"){const e=/** @type {HTMLButtonElement|HTMLInputElement|null} */t.closest('input[type="submit"], button');if(e&&e.form&&e.type==="submit")return true;const n=t.closest("a");const r=/^#.+/;if(n&&n.href&&!r.test(n.getAttribute("href")))return true}return false}
/**
   * @param {Node} elt
...
/**
   ...
   */function getRelatedFormData(e){const t=getTargetButton(e.target);if(!t)return;const n=getRelatedForm(t);return n?getInternalData(n):void 0}
/**
   * @param {EventTarget} elt
...
/** @type HtmxHistoryItem */const i={url:e,content:n,title:r,scroll:o};triggerEvent(getDocument().body,"htmx:historyItemCreated",{item:i,cache:s});s.push(i);while(s.length>htmx.config.historyCacheSize)s.shift();while(s.length>0)try{sessionStorage.setItem("htmx-history-cache",JSON.stringify(s));break}catch(e){triggerErrorEvent(getDocument().body,"htmx:historyCacheError",{cause:e,cache:s});s.shift()}}
/**
...
/**
   * @param {Element} elt
   * @param {HtmxElementValidationError[]} errors
   */function validateElement(e,t){const n=/** @type {HTMLElement & ElementInternals} */e;if(n.willValidate){triggerEvent(n,"htmx:validation:validate");if(!n.checkValidity()){triggerEvent(n,"htmx:validation:failed",{message:n.validationMessage,validity:n.validity})&&!t.length&&htmx.config.reportValidityOfForms&&n.reportValidity();t.push({elt:n,message:n.validationMessage,validity:n.validity})}}}
/**
...
/** @type HtmxElementValidationError[] */const s=[];const i=getInternalData(e);i.lastButtonClicked&&!bodyContains(i.lastButtonClicked)&&(i.lastButtonClicked=null);let a=e instanceof HTMLFormElement&&e.noValidate!==true||getAttributeValue(e,"hx-validate")==="true";i.lastButtonClicked&&(a=a&&i.lastButtonClicked.formNoValidate!==true);t!=="get"&&processInputValue(n,o,s,getRelatedForm(e),a);processInputValue(n,r,s,e,a);if(i.lastButtonClicked||e.tagName==="BUTTON"||e.tagName==="INPUT"&&getRawAttribute(e,"type")==="submit"){const t=i.lastButtonClicked||/** @type HTMLInputElement|HTMLButtonElement */e;const n=getRawAttribute(t,"name");addValueToFormData(n,t.value,o)}const l=findAttributeTargets(e,"hx-include");forEach(l,(function(e){processInputValue(n,r,s,asElement(e),a);matches(e,"form")||forEach(asParentNode(e).querySelectorAll(INPUT_SELECTOR),(function(e){processInputValue(n,r,s,e,a)}))}));overrideFormData(r,o);return{errors:s,formData:r,values:formDataProxy(r)}}
/**
 ...
 * @param {Event=} event
 * @returns {Object}
 */function getValuesForElement(e,t,n,r,o){r==null&&(r={});if(e==null)return r;const s=getAttributeValue(e,t);if(s){let t=s.trim();let i=n;if(t==="unset")return null;if(t.indexOf("javascript:")===0){t=t.slice(11);i=true}else if(t.indexOf("js:")===0){t=t.slice(3);i=true}t.indexOf("{")!==0&&(t="{"+t+"}");let a;a=i?maybeEval(e,(function(){return o?Function("event","return ("+t+")").call(e,o):Function("return ("+t+")").call(e)}),{}):parseJSON(t);for(const e in a)a.hasOwnProperty(e)&&r[e]==null&&(r[e]=a[e])}return getValuesForElement(asElement(parentElt(e)),t,n,r,o)}
/**
   * @param {EventTarget|string} elt
   * @param {() => any} toEval
...
/**
   ...
   */function ajaxHelper(e,t,n){e=/** @type HttpVerb */e.toLowerCase();if(n){if(n instanceof Element||typeof n==="string")return issueAjaxRequest(e,t,null,null,{targetOverride:resolveTarget(n)||DUMMY_ELT,returnPromise:true});{let r=resolveTarget(n.target);(n.target&&!r||n.source&&!r&&!resolveTarget(n.source))&&(r=DUMMY_ELT);return issueAjaxRequest(e,t,resolveTarget(n.source),n.event,{handler:n.handler,headers:n.headers,values:n.values,targetOverride:r,swapOverride:n.swap,select:n.select,returnPromise:true,push:n.push,replace:n.replace,selectOOB:n.selectOOB})}}return issueAjaxRequest(e,t,null,null,{returnPromise:true})}
/**
   * @param {Element} elt
   * @return {Element[]}
...
/**
   ...
   * @return {Promise<void>}
   */function issueAjaxRequest(e,t,n,r,o,s){let i=null;let a=null;o=o!=null?o:{};if(o.returnPromise&&typeof Promise!=="undefined")var l=new Promise((function(e,t){i=e;a=t}));n==null&&(n=getDocument().body);const c=o.handler||handleAjaxResponse;const u=o.select||null;if(!bodyContains(n)){maybeCall(i);return l}const f=o.targetOverride||asElement(getTarget(n));if(f==null||f==DUMMY_ELT){triggerErrorEvent(n,"htmx:targetError",{target:getClosestAttributeValue(n,"hx-target")});maybeCall(a);return l}let g=getInternalData(n);const d=g.lastButtonClicked;if(d){const n=getRawAttribute(d,"formaction");n!=null&&(t=n);const r=getRawAttribute(d,"formmethod");if(r!=null){if(!VERBS.includes(r.toLowerCase())){maybeCall(i);return l}e=/** @type HttpVerb */r}}const h=getClosestAttributeValue(n,"hx-confirm");if(s===void 0){const s=function(s){return issueAjaxRequest(e,t,n,r,o,!!s)};const a={target:f,elt:n,path:t,verb:e,triggeringEvent:r,etc:o,issueRequest:s,question:h};if(triggerEvent(n,"htmx:confirm",a)===false){maybeCall(i);return l}}let m=n;let p=getClosestAttributeValue(n,"hx-sync");let E=null;let x=false;if(p){const e=p.split(":");const t=e[0].trim();m=t==="this"?findThisElement(n,"hx-sync"):asElement(querySelectorExt(n,t));p=(e[1]||"drop").trim();g=getInternalData(m);if(p==="drop"&&g.xhr&&g.abortable!==true){maybeCall(i);return l}if(p==="abort"){if(g.xhr){maybeCall(i);return l}x=true}else if(p==="replace")triggerEvent(m,"htmx:abort");else if(p.indexOf("queue")===0){const e=p.split(" ");E=(e[1]||"last").trim()}}if(g.xhr){if(!g.abortable){if(E==null){if(r){const e=getInternalData(r);e&&e.triggerSpec&&e.triggerSpec.queue&&(E=e.triggerSpec.queue)}E==null&&(E="last")}g.queuedRequests==null&&(g.queuedRequests=[]);if(E==="first"&&g.queuedRequests.length===0)g.queuedRequests.push((function(){issueAjaxRequest(e,t,n,r,o)}));else if(E==="all")g.queuedRequests.push((function(){issueAjaxRequest(e,t,n,r,o)}));else if(E==="last"){g.queuedRequests=[];g.queuedRequests.push((function(){issueAjaxRequest(e,t,n,r,o)}))}maybeCall(i);return l}triggerEvent(m,"htmx:abort")}const y=new XMLHttpRequest;g.xhr=y;g.abortable=x;const b=function(){g.xhr=null;g.abortable=false;if(g.queuedRequests!=null&&g.queuedRequests.length>0){const e=g.queuedRequests.shift();e()}};const v=getClosestAttributeValue(n,"hx-prompt");if(v){var S=prompt(v);if(S===null||!triggerEvent(n,"htmx:prompt",{prompt:S,target:f})){maybeCall(i);b();return l}}if(h&&!s&&!confirm(h)){maybeCall(i);b();return l}let w=getHeaders(n,f,S);e==="get"||usesFormData(n)||(w["Content-Type"]="application/x-www-form-urlencoded");o.headers&&(w=mergeObjects(w,o.headers));const C=getInputValues(n,e);let A=C.errors;const T=C.formData;o.values&&overrideFormData(T,formDataFromObject(o.values));const H=formDataFromObject(getExpressionVars(n,r));const O=overrideFormData(T,H);let R=filterValues(O,n);htmx.config.getCacheBusterParam&&e==="get"&&R.set("org.htmx.cache-buster",getRawAttribute(f,"id")||"true");t!=null&&t!==""||(t=location.href)
/**
     * @type {Object}
     * @property {boolean} [credentials]
     * @property {number} [timeout]
     * @property {boolean} [noHeaders]
     */;const I=getValuesForElement(n,"hx-request");const D=getInternalData(n).boosted;let N=htmx.config.methodsThatUseUrlParams.indexOf(e)>=0;
/** @type HtmxRequestConfig */const L={boosted:D,useUrlParams:N,formData:R,parameters:formDataProxy(R),unfilteredFormData:O,unfilteredParameters:formDataProxy(O),headers:w,elt:n,target:f,verb:e,errors:A,withCredentials:o.credentials||I.credentials||htmx.config.withCredentials,timeout:o.timeout||I.timeout||htmx.config.timeout,path:t,triggeringEvent:r};if(!triggerEvent(n,"htmx:configRequest",L)){maybeCall(i);b();return l}t=L.path;e=L.verb;w=L.headers;R=formDataFromObject(L.parameters);A=L.errors;N=L.useUrlParams;if(A&&A.length>0){triggerEvent(n,"htmx:validation:halted",L);maybeCall(i);b();return l}const F=t.split("#");const P=F[0];const q=F[1];let V=t;if(N){V=P;const e=!R.keys().next().done;if(e){V.indexOf("?")<0?V+="?":V+="&";V+=urlEncode(R);q&&(V+="#"+q)}}if(!verifyPath(n,V,L)){triggerErrorEvent(n,"htmx:invalidPath",L);maybeCall(a);b();return l}y.open(e.toUpperCase(),V,true);y.overrideMimeType("text/html");y.withCredentials=L.withCredentials;y.timeout=L.timeout;if(I.noHeaders);else for(const e in w)if(w.hasOwnProperty(e)){const t=w[e];safelySetHeaderValue(y,e,t)}
/** @type {HtmxResponseInfo} */const M={xhr:y,target:f,requestConfig:L,etc:o,boosted:D,select:u,pathInfo:{requestPath:t,finalRequestPath:V,responsePath:null,anchor:q}};y.onload=function(){try{const e=hierarchyForElt(n);M.pathInfo.responsePath=getPathFromResponse(y);c(n,M);M.keepIndicators!==true&&removeRequestIndicators(k,B);triggerEvent(n,"htmx:afterRequest",M);triggerEvent(n,"htmx:afterOnLoad",M);if(!bodyContains(n)){let t=null;while(e.length>0&&t==null){const n=e.shift();bodyContains(n)&&(t=n)}if(t){triggerEvent(t,"htmx:afterRequest",M);triggerEvent(t,"htmx:afterOnLoad",M)}}maybeCall(i)}catch(e){triggerErrorEvent(n,"htmx:onLoadError",mergeObjects({error:e},M));throw e}finally{b()}};y.onerror=function(){removeRequestIndicators(k,B);triggerErrorEvent(n,"htmx:afterRequest",M);triggerErrorEvent(n,"htmx:sendError",M);maybeCall(a);b()};y.onabort=function(){removeRequestIndicators(k,B);triggerErrorEvent(n,"htmx:afterRequest",M);triggerErrorEvent(n,"htmx:sendAbort",M);maybeCall(a);b()};y.ontimeout=function(){removeRequestIndicators(k,B);triggerErrorEvent(n,"htmx:afterRequest",M);triggerErrorEvent(n,"htmx:timeout",M);maybeCall(a);b()};if(!triggerEvent(n,"htmx:beforeRequest",M)){maybeCall(i);b();return l}var k=addRequestIndicatorClasses(n);var B=disableElements(n);forEach(["loadstart","loadend","progress","abort"],(function(e){forEach([y,y.upload],(function(t){t.addEventListener(e,(function(t){triggerEvent(n,"htmx:xhr:"+e,{lengthComputable:t.lengthComputable,loaded:t.loaded,total:t.total})}))}))}));triggerEvent(n,"htmx:beforeSend",M);const U=N?null:encodeParamsForBody(y,n,R);y.send(U);return l}
/**
...
/**
   ...
   */function determineHistoryUpdates(e,t){const n=t.xhr;let r=null;let o=null;if(hasHeader(n,/HX-Push:/i)){r=n.getResponseHeader("HX-Push");o="push"}else if(hasHeader(n,/HX-Push-Url:/i)){r=n.getResponseHeader("HX-Push-Url");o="push"}else if(hasHeader(n,/HX-Replace-Url:/i)){r=n.getResponseHeader("HX-Replace-Url");o="replace"}if(r)return r==="false"?{}:{type:o,path:r};const s=t.pathInfo.finalRequestPath;const i=t.pathInfo.responsePath;const a=t.etc.push||getClosestAttributeValue(e,"hx-push-url");const l=t.etc.replace||getClosestAttributeValue(e,"hx-replace-url");const c=getInternalData(e).boosted;let u=null;let f=null;if(a){u="push";f=a}else if(l){u="replace";f=l}else if(c){u="push";f=i||s}if(f){if(f==="false")return{};f==="true"&&(f=i||s);t.pathInfo.anchor&&f.indexOf("#")===-1&&(f=f+"#"+t.pathInfo.anchor);return{type:u,path:f}}return{}}
/**
...
/**
   ...
   * @param {string[]=} extensionsToIgnore
   * @returns {HtmxExtension[]}
   */function getExtensions(e,t,n){t==void 0&&(t=[]);if(e==void 0)return t;n==void 0&&(n=[]);const r=getAttributeValue(e,"hx-ext");r&&forEach(r.split(","),(function(e){e=e.replace(/ /g,"");if(e.slice(0,7)!="ignore:"){if(n.indexOf(e)<0){const n=extensions[e];n&&t.indexOf(n)<0&&t.push(n)}}else n.push(e.slice(7))}));return getExtensions(asElement(parentElt(e)),t,n)}var isReady=false;getDocument().addEventListener("DOMContentLoaded",(function(){isReady=true}));function ready(e){isReady||getDocument().readyState==="complete"?e():getDocument().addEventListener("DOMContentLoaded",e)}function insertIndicatorStyles(){if(htmx.config.includeIndicatorStyles!==false){const e=htmx.config.inlineStyleNonce?` nonce="${htmx.config.inlineStyleNonce}"`:"";const t=htmx.config.indicatorClass;const n=htmx.config.requestClass;getDocument().head.insertAdjacentHTML("beforeend",`<style${e}>.${t}{opacity:0;visibility: hidden} .${n} .${t}, .${n}.${t}{opacity:1;visibility: visible;transition: opacity 200ms ease-in}</style>`)}}function getMetaConfig(){
/** @type HTMLMetaElement */
const e=getDocument().querySelector('meta[name="htmx-config"]');return e?parseJSON(e.content):null}function mergeMetaConfig(){const e=getMetaConfig();e&&(htmx.config=mergeObjects(htmx.config,e))}ready((function(){mergeMetaConfig();insertIndicatorStyles();let e=getDocument().body;processNode(e);const t=getDocument().querySelectorAll("[hx-trigger='restored'],[data-hx-trigger='restored']");e.addEventListener("htmx:abort",(function(e){const t=/** @type {CustomEvent} */e.detail.elt||e.target;const n=getInternalData(t);n&&n.xhr&&n.xhr.abort()}));
